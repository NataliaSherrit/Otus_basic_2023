...
public class ConverterValueToString {
    public String convert(Currency currency, Integer number) {
        String resultString;
        String numberStr;
        String form;
       if (number <= 0) {
            throw new IllegalArgumentException("Сумма не может быть меньше или равна 0");
        } else {
    ...
    public String calculatePartOfNumber(int number){
        String numberStr;
        if (number > 0 && number < 1000) {
            numberStr = convertToString(number);
        } else if (number >= 1000 && number < 1000000){
            String thousand;
            int countThousand = number / 1000;
            ...
            int countMillion = number / 1000000;
    ...
    public String calculatePartNumber(int number, int divider, String[] formNumber){
        String s;
        if (number % divider == 0) {
    ...
    public String getDeclension(int count, int type){
        String form = " ";
